...
const DetailsWrapper = ({ productItem, handleImageActive, activeImg, detailsBottom = false }) => {
  ...
  const { sku, img, name, featuredImage,galleryImage, category, description, discount, price, status, reviews, tags, offerExpiryTime } = productItem || {};
  ...
  return (
    <div className="tp-product-details-wrapper">
      ...
      <div className="tp-product-details-price-wrapper mb-20">
        {discount > 0 ? (
          <>
            <span className="tp-product-details-price old-price">${price?.levelTwo}</span>
            <span className="tp-product-details-price new-price">
              {" "}${(Number(price?.levelTwo) - (Number(price?.levelTwo) * Number(discount)) / 100).toFixed(2)}
            </span>
          </>
        ) : (
          <span className="tp-product-details-price new-price">${price?.levelTwo.toFixed(2)}</span>
        )}
      </div>

      {/* variations */}
      {/* {galleryImage && <div className="tp-product-details-variation">
        <div className="tp-product-details-variation-item">
          <h4 className="tp-product-details-variation-title">Color :</h4>
          <div className="tp-product-details-variation-list">
            {galleryImage.map((item, i) => (
              <button onClick={() => handleImageActive(item)} key={i} type="button"
                className={`color tp-color-variation-btn ${item === activeImg ? "active" : ""}`} >
                <span data-bg-color={`${item}`}
                  style={{ backgroundColor: `${item}` }}></span>
                <span className="tp-color-variation-tootltip">{item}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      {offerExpiryTime && <ProductDetailsCountdown offerExpiryTime={offerExpiryTime} />}
