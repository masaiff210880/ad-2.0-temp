...
const ProductSmArea = () => {
  const { data: products, isError, isLoading, refetch } = useGetProductTypeQuery({type:'Smoke'});
  ...
  if (!isLoading && !isError && products?.data?.length > 0) {
    // const discount_prd = products.data.filter(p => p.brand > 0).slice(0, 3);
    ...
    const discount_prd = products.data.slice().sort((a, b) => b.sellCount - a.sellCount).slice(0, 3);
    const featured_prd = products.data.slice().sort((a, b) => b.sellCount - a.sellCount).slice(0, 3);
    // console.log('all arival products',discount_prd,featured_prd,selling_prd)
  ...
  // console.log('arrivals products',products)
  return (
