...
// Login Method
export const postFakeLogin = (data) =>
...
// Ecommerce
// get Products
// export const getProducts = (limit) => api.get(url.GET_PRODUCTS +`?limit=${limit}`);

export const getProducts = (limit) => {
  // console.log("Limit:", limit);
  return api.get(url.GET_PRODUCTS + `?limit=${limit}`);
};

// get Products
export const getProductsBySearch = (name) =>
  api.get(url.GET_PRODUCTS_BY_SEARCH + "/" + name);

export const getProductDisable = (id, disable) =>
  api.patch(url.UPDATE_PRODUCT_STATUS + "/" + id, disable);
// delete Product
...
export const getSingleProduct = (id) =>
  api.get(url.GET_SINGLE_PRODUCT + "/" + id);
// update Products
// export const updateProduct = (product) =>
//   api.update(url.UPDATE_PRODUCT + "/" + product._id, product);

export const updateProductData = (id, product) =>
  api.update(url.UPDATE_PRODUCT_DETAILS + "/" + id, product);
// + "/" + id, customer

//Category
export const addNewCategory = (category) => {
  api.create(url.POST_CREATE_CATEGORY, category);
};

export const getProductCategory = () => {
  return api.get(url.GET_PRODUCT_CATEGORY);
};
//Vendor
export const getAllVendor = () => {
  return api.get(url.GET_ALL_VENDOR);
};
//Vendor
export const getAllVendorPO = (id) => {
  return api.get(url.GET_ALL_VENDOR_QUERY + `?vendorId=${id}`);
};
export const getSingleVendorData = (id) => {
  return api.get(url.GET_SINGLE_VENDOR + "/" + id);
};
export const getPurchaseReceiptData = (id) => {
  return api.get(url.GET_PURCHASE_RECEIPT_DATA + `?vendorId=${id}`);
};
export const getAllDisburshmentData = (id) => {
  return api.get(url.GET_ALL_DISBURSHMENT_DATA + `?vendorId=${id}`);
};
export const getAllVendorActivity = (id) => {
  return api.get(url.GET_ALL_VENDOR_ACTIVITY + `?vendorId=${id}`);
};
export const getCustomerRmasData = (id) => {
  return api.get(url.GET_CSTOMER_RMAS + `?userId=${id}`);
};
export const deleteSingleVendor = (vendorId) => {
  return api.delete(url.DELETE_ONE_VENDOR + `/${vendorId}`);
};
export const getPurchaseSalesOrderData = (vendorSalesId) => {
  return api.get(url.GET_ONE_PURCHASE_ORDER + `/${vendorSalesId}`);
};
//Customer
export const getAllCustomer = () => {
  return api.get(url.GET_ALL_CUSTOMER);
};
export const getSingleCustomer = (id) => {
  return api.get(url.GET_SINGLE_CUSTOMER + "/" + id);
};
export const getSalesOrderData = (id) => {
  return api.get(url.GET_SALES_ORDER + `?userId=${id}`);
};
export const getPayments = (id) => {
  return api.get(url.GET_ALL_PAYMENT + `?userId=${id}`);
};
export const addNewCustomer = (customer) =>
  api.create(url.POST_CREATE_CUSTOMER, customer);

export const updateCustomerData = (id, customer) =>
  api.update(url.UPDATE_CUSTOMER_DETAILS + "/" + id, customer);

export const getAllCustomerActivity = (id) => {
  return api.get(url.GET_ALL_ACTIVITY + `?userId=${id}`);
};

export const deleteCustomers = (id) => {
  return api.delete(url.DELETE_ONE_CUSTOMER + `/${id}`);
};

//Sales Order
export const getSallerData = () => api.get(url.GET_SALES_ORDER);

// One Sales Order
export const getOneSalesData = (salesId) =>
  api.get(url.GET_ONE_SALES_ORDERS + `/${salesId}`);
...
// get ProductsVendors
export const getProductsVendors = (productId) => {
  // console.log("Limit:", limit);
  return api.get(url.GET_PRODUCTS_VENDORS + `/${productId}`);
};
